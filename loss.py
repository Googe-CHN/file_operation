...
Loss_rpn_box_reg = []
with open('/home/birl/Data/PoleDet/results/1021-augmentation/log.txt','r') as file:  #打开文件
    for line in file.readlines():    #文件内容分析成一个行的列表
        line = line.strip().split(" ")   #按照空格进行切分
        #print(line)
        itera, loss, loss_box_reg, loss_classifier, loss_ke, loss_mask, loss_mty, loss_objectness, loss_rpn_box_reg = \
            line[8], line[11], line[15], line[19], line[23], line[27], line[31], line[35], line[39]    #一行拆分为三行
        ...
        loss_ke = float(loss_ke)
        Loss_ke.append(loss_ke)

        loss_mask = float(loss_mask)
        Loss_mask.append(loss_mask)

        loss_mty = float(loss_mty)
        Loss_mty.append(loss_mty)
...
plt.plot(iter_loss, Loss_ke, color='deeppink', label='Loss_ke')
plt.plot(iter_loss, Loss_mask, color='firebrick', label='Loss_mask')
plt.plot(iter_loss, Loss_mty, color='goldenrod', label='Loss_mty')
